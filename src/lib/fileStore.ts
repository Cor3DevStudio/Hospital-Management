...
export async function listFiles(): Promise<{ key: string; filename: string; mime: string; size: number; createdAt: string }[]> {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, "readonly");
    const store = tx.objectStore(STORE_NAME);
    ...
    r.onsuccess = () => {
      resolve((r.result || []).map((e: { key: string; filename: string; mime: string; size: number; createdAt: string }) => ({
        key: e.key,
        filename: e.filename,
        mime: e.mime,
        size: e.size,
