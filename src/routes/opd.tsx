...
function OPDPage() {
  ...
  useResetPageOnChange(allVisitsList.resetPage, [
    tab,
    allQuery,
    state.consultations.length,
    state.opdRecords.length,
  ]);
  ...
  const addRx = () =>
    setForm({
      ...form,
      prescriptions: [...form.prescriptions, { medicine: "", dosage: "", instructions: "" }],
    });
  ...
  return (
    <>
      ...
      <div className="no-print h-[calc(100vh-3rem)] flex flex-col overflow-hidden bg-background">
        <PageHeader
          title="OPD"
          description="Document outpatient visits, prescriptions, and discharge status."
        />
              ...
              <CardHeader className="shrink-0 space-y-3 pb-3 pt-4 px-4">
                <TabsList className="h-8 w-full">
                  ...
                  <TabsTrigger value="all" className="text-xs flex-1">
                    All Records
                    {allVisitsList.totalItems > 0 ? ` (${allVisitsList.totalItems})` : ""}
                  </TabsTrigger>
                </TabsList>
                {tab === "all" && (
                  <Input
                      ...
                      <TableBody>
                        {allVisitsList.totalItems === 0 ? (
                          <TableRow>
                            ...
                            >
                              {allQuery.trim()
                                ? "No OPD records match your search"
                                : "No OPD records yet"}
                            </TableCell>
                          </TableRow>
                        ) : (
          ...
          <Card className="flex min-h-0 min-w-0 flex-col overflow-hidden">
            <CardHeader className="shrink-0 pb-2 pt-4 px-4">
              <CardTitle className="text-base">
                {editId ? "Edit OPD Visit" : "New OPD Visit"}
              </CardTitle>
            </CardHeader>
            <CardContent className="flex min-h-0 flex-1 flex-col justify-between space-y-4 overflow-y-auto border-t p-4 pt-3">
              {isLocked && (
                <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                  This OPD visit has been seen and is read-only. Contact an Administrator to make
                  changes.
                </p>
              )}
              <fieldset
                ...
                {linkedAppointments.length > 0 && (
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setForm({ ...form, appointmentId: v === "__none" ? undefined : v })
                      }
                    >
                      <SelectTrigger className="h-9">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {form.prescriptions.map((rx, i) => (
                      <div
                        key={i}
                        className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_1.5fr_auto]"
                      >
                        <Input
                          className="h-9 text-xs"
                    ...
                    {form.prescriptions.length === 0 && (
                      <p className="text-xs text-muted-foreground">No prescriptions added.</p>
                    )}
                  </div>
                </div>
              </fieldset>
              ...
              <div className="mt-4 flex shrink-0 flex-wrap justify-end gap-2 border-t pt-3">
                ...
                <Button size="sm" onClick={save} disabled={isLocked}>
                  <Save className="h-3.5 w-3.5" /> Save OPD Visit
                </Button>
              </div>
            </CardContent>
          </Card>
