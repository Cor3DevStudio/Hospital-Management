...
} from "@/components/clinical/clinicalPrintStyles";
import {
...
function AdmissionPage() {
  ...
  const save = () => {
    if (isLocked)
      return toast.error(
        "This admission is discharged and read-only. Contact an Administrator to make changes.",
      );
    ...
    if (!editId && isPatientAdmitted(state, form.patientId)) {
      return toast.error(
        "Patient is still admitted. Discharge first before creating a new admission record.",
      );
    }
    const payload = {
      ...form,
  ...
  return (
    <>
      ...
      <div className="no-print h-[calc(100vh-3rem)] flex flex-col overflow-hidden bg-background">
        <PageHeader
          title="Admissions"
          description="Track inpatient admissions, room assignments, and discharge status."
        />
        <div className="flex-1 grid gap-4 p-4 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] items-stretch min-h-0 overflow-hidden">
            ...
            <Card className="flex flex-col shrink-0">
              <CardHeader className="pb-2 pt-4 px-4">
                <CardTitle className="text-base">
                  {editId ? "Edit Admission" : "New Admission"}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 p-4 pt-2 border-t flex flex-col justify-between">
                {isLocked && (
                  <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                    This record is discharged and read-only. Contact an Administrator to make
                    changes.
                  </p>
                )}
                <fieldset
                  disabled={isLocked}
                  className="m-0 border-0 p-0 space-y-4 disabled:opacity-60"
                >
                          ...
                          {roomRates.length === 0 ? (
                            ...
                            roomRates.map((r) => (
                              <SelectItem key={r.id} value={r.id}>
                                {r.description} — ₱
                                {(
                                  getPriceAsOf(state, "priceItem", r.id, form.admissionDate) ??
                                  r.caseRate
                                ).toLocaleString()}
                                /day
                              </SelectItem>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setForm({ ...form, dischargeDate: e.target.value || undefined })
                        }
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setForm({ ...form, admissionType: value as Admission["admissionType"] })
                        }
                      >
                        <SelectTrigger className="h-9">
                          <SelectValue />
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        value={form.status}
                        onValueChange={(value) =>
                          setForm({ ...form, status: value as Admission["status"] })
                        }
                      >
                        <SelectTrigger className="h-9">
                          <SelectValue />
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setForm({ ...form, attendingDoctor: value })}
                    >
                      <SelectTrigger className="h-9">
                  ...
                  {(form.roomStays?.length ?? 0) > 0 && (
                    <div className="space-y-1">
                              ...
                              return (
                                <TableRow key={stay.id}>
                                  <TableCell className="text-xs">
                                    {roomTypeLabel(state, stay.roomTypeId) || stay.roomWard}
                                  </TableCell>
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        Room & Board is auto-posted on discharge (days × rate as of each stay start
                        date).
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      <Input
                        ...
                        onChange={async (e) => {
                          ...
                          if (!form.id)
                            return toast.error(
                              "Please save the admission record first to attach files.",
                            );
                          try {
                      ...
                      <div className="space-y-2">
                        {((state.attachments || []) as any).filter(
                          (a: any) => a.refType === "admission" && a.refId === form.id,
                        ).length === 0 && (
                          <p className="text-sm text-muted-foreground">
                            No documents attached to this admission.
                          </p>
                        )}
                        {((state.attachments || []) as any)
                          .filter((a: any) => a.refType === "admission" && a.refId === form.id)
                          .map((a: any) => (
                            <div
                              key={a.id}
                ...
                <div className="flex flex-wrap justify-end gap-2 border-t pt-3 mt-4 shrink-0">
                  {editId && form.status === "Admitted" && !isLocked && (
                    <Button variant="outline" size="sm" onClick={handleDischarge}>
                      Discharge
                    </Button>
                  )}
                  {editId && form.status === "Discharged" && !isLocked && (
                    <Button variant="outline" size="sm" onClick={handleCancelDischarge}>
                      Cancel Discharge
                    </Button>
                  )}
                  {canPrintChart && (
                  ...
                  <Button size="sm" onClick={save} disabled={isLocked}>
          ...
          <Card className="flex flex-col h-full min-h-0">
            ...
            <CardContent className="flex-1 overflow-y-auto p-0 border-t">
                    ...
                    admList.pageItems.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="pl-4 text-xs sm:text-sm">
                          {patientMap.get(item.patientId)?.lastName ?? "Unknown"}
                        </TableCell>
              ...
              <ListPagination
                page={admList.page}
                totalPages={admList.totalPages}
                totalItems={admList.totalItems}
                onPageChange={admList.setPage}
              />
            </CardContent>
          </Card>
        </div>
