...
function BillingPage() {
  ...
  const cancelDischargeCheck = selectedBill
    ? canCancelBillDischarge(state, selectedBill)
    : { allowed: false };
  ...
  const availablePriceItems = useMemo(
    () => state.prices.map((p) => ({ value: p.id, label: `${p.code} — ${p.description}` })),
    [state.prices],
  );
  const availableMedicines = useMemo(
    () =>
      state.medicines
        .filter((m) => !m.archived)
        .map((m) => ({ value: m.id, label: `${m.name} (${m.stock} ${m.unit || "pcs"})` })),
    [state.medicines],
  );
  const selectedPriceItemAmount =
    selectedPriceItemId !== "none"
      ? getPriceAsOf(state, "priceItem", selectedPriceItemId, billingAsOfDate)
      : undefined;
  const selectedMedicineAmount =
    selectedMedicineId !== "none"
      ? getPriceAsOf(state, "medicine", selectedMedicineId, billingAsOfDate)
      : undefined;
  ...
  const handleAddItem = () => {
    ...
    if (!inputDesc && !priceItemId && !medicineId)
      return toast.error("Item Description is required");
    ...
    const description = inputDesc || priceItem?.description || medicine?.name || "";
          ...
          bills: next.bills.map((b) =>
            b.id === selectedBill.id
              ? {
                  ...b,
                  notes: notes || b.notes,
                  dischargeDate: dischargeDate || b.dischargeDate,
  ...
  const billingOfficerName =
    state.users.find((u) => u.username === state.authedUser)?.fullName || "System Administrator";
  ...
  return (
    <>
      ...
      <div className="h-[calc(100vh-3rem)] flex flex-col overflow-hidden bg-background no-print">
        <PageHeader
          title="Billing"
          description="Create bills, apply PhilHealth benefits, collect payments, and discharge patients."
        />
                  ...
                  {patientBillList.pageItems.map((b) => (
                    ...
                    >
                      <div className="flex justify-between items-center w-full">
                        ...
                        <Badge
                          className={`text-[9px] font-bold px-1.5 py-0 ${b.status === "Paid" ? "bg-emerald-100 text-emerald-800 border-emerald-300" : b.status === "Partial" ? "bg-amber-100 text-amber-800 border-amber-300" : "bg-red-100 text-red-800 border-red-300"}`}
                          variant="outline"
                        >
                          {b.status}
                        </Badge>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Total: ₱
                        {(
                          b.items.reduce((s, i) => s + (i.amount || 0), 0) - b.philhealthDeduction
                        ).toLocaleString()}{" "}
              ...
              <CardContent className="space-y-3 p-3 pt-1">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Laboratory Panel, Pharmacy stock..."
                    className="h-9"
                  />
                </div>
                <div className="grid gap-2 md:grid-cols-[1.5fr_1fr]">
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(val) => {
                        setSelectedPriceItemId(val);
                        if (val !== "none") setSelectedMedicineId("none");
                      }}
                    >
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Select a price item" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(val) => {
                        ...
                        if (val !== "none") setSelectedPriceItemId("none");
                      }}
                    >
                      <SelectTrigger className="h-9">
                ...
                {(selectedPriceItemAmount !== undefined ||
                  selectedMedicineAmount !== undefined) && (
                  <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
                    <p className="font-semibold">Resolved Unit Price</p>
                    <p>
                      {selectedPriceItemAmount !== undefined
                        ? `Price item unit rate: ₱${selectedPriceItemAmount.toFixed(2)} as of ${billingAsOfDate}`
                        : selectedMedicineAmount !== undefined
                          ? `Medicine unit price: ₱${selectedMedicineAmount.toFixed(2)} as of ${billingAsOfDate}`
                          : ""}
                    </p>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      type="number"
                      value={inputQty}
                      onChange={(e) => setInputQty(Math.max(1, +e.target.value))}
                      className="h-9"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2 rounded-md border p-2.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        value={doctorAmount}
                        onChange={(e) => setDoctorAmount(e.target.value)}
                        placeholder="0.00"
                        className="h-9"
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Select
                        value={doctorRole}
                        onValueChange={(v) => setDoctorRole(v as "Surgeon" | "Anesthesiologist")}
                      >
                        <SelectTrigger className="h-9">
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 w-full text-xs text-blue-600 border-blue-200 hover:bg-blue-50"
              ...
              <CardContent className="flex-1 overflow-hidden p-3 pt-1 border-t flex flex-col justify-between min-h-0">
                <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
                  {draftItems.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center rounded-md border p-2.5 bg-muted/20 text-xs"
                    >
                      <div>
                        <p className="font-semibold text-slate-800">{item.description}</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">
                          {item.category || "Other"}
                          {item.doctorRole ? ` (${item.doctorRole})` : ""} ·{" "}
                          {item.effectiveDate || billingAsOfDate} · Qty {item.qty} × ₱
                  ...
                  {draftItems.length === 0 && (
                    <p className="text-xs text-muted-foreground py-4 text-center">
                      No draft charges. Add items above.
                    </p>
                  )}
                </div>
                ...
                <div className="flex justify-end pt-1.5 shrink-0">
                  <p className="text-sm font-bold text-blue-600">
                    Draft Total: ₱
                    {draftTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent className="flex-1 overflow-y-auto space-y-4 p-4 pt-2 border-t flex flex-col justify-between min-h-0">
              {selectedBill ? (
                  ...
                  <div className="space-y-3.5">
                    <div className="rounded border p-2.5 text-xs bg-blue-50/50 space-y-1">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Active Statement:</span>
                        <span className="font-mono font-semibold">{selectedBill.id}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select value={paymentMethod} onValueChange={setPaymentMethod}>
                          ...
                          <SelectContent>
                            {["Cash", "GCash", "PayMaya", "Credit Card", "Bank Transfer"].map(
                              (m) => (
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs text-muted-foreground font-semibold flex items-center justify-between">
                        ...
                        {dischargeDate && (
                          <Badge
                            variant="outline"
                            className="text-[9px] font-bold px-1.5 py-0 bg-emerald-50 text-emerald-800 border-emerald-300"
                          >
                            Discharged
                          </Badge>
                        )}
                    ...
                    <div className="rounded-md border bg-muted/30 p-2.5 text-xs space-y-1">
                      ...
                      <div className="flex justify-between text-muted-foreground">
                        ...
                        <span>
                          − ₱
                          {selectedBill.philhealthDeduction.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </span>
                      </div>
                      <div className="flex justify-between font-semibold">
                        ...
                        <span>
                          ₱
                          {selectedBillTotal.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </span>
                      </div>
                      <div className="flex justify-between text-muted-foreground">
                        ...
                        <span>
                          ₱
                          {selectedBill.amountPaid.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs font-bold border-t pt-1 mt-1">
                        ...
                        <span className={selectedBillBalance > 0 ? "text-red-600" : "text-black"}>
                          ₱
                          {selectedBillBalance.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </span>
                      </div>
                    ...
                    <div className="mt-2">
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        <div className="space-y-2">
                          {((state.attachments || []) as any).filter(
                            (a: any) => a.refType === "bill" && a.refId === selectedBill.id,
                          ).length === 0 && (
                          ...
                          {((state.attachments || []) as any)
                            .filter((a: any) => a.refType === "bill" && a.refId === selectedBill.id)
                            .map((a: any) => (
                              <div
                                key={a.id}
                  ...
                  <div className="space-y-2 border-t pt-3 mt-4 shrink-0">
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        ...
                        onClick={handleApplyPhilHealthPay}
                      >
                        Apply PhilHealth & Pay
                      </Button>
                      <Button
                        size="sm"
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-xs text-muted-foreground">
                  <p>
                    Select a bill from the patient's Billing History on the left to process
                    payments, apply case rates, and print statements.
                  </p>
                </div>
              )}
            </CardContent>
